...
from distributionalDQN_learner import learner
import numpy as np
import torch
...
def parse_args():
    ...
    parser.add_argument("--train-model", type=int, default=1, help="train or test,train is 1,test is 2,test per train episode is 3")
    parser.add_argument("--num-episodes", type=int, default=6, help="number of episodes")
...
def train(arglist):
    ...
    agent=learner(env.observation_space.shape[0],
            ...
            arglist.end_epsilon,
            # when gamma >= 0.98,
            # RuntimeError: invalid argument 2: out of range at 
            # /Users/distiller/project/conda/conda-bld/pytorch_1556653557089/work/aten/src/TH/generic/THTensor.cpp:639
            # will occurs.
            # fix later
            arglist.gamma,
            )
    ...
    with open('results.txt','wb') as results:
        ...
        for epi in range(1,arglist.num_episodes+1):
            ...
            while not done:
                ...
                if train_agent.memory.size>train_agent.batch_size:
