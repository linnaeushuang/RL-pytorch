...
from memory import ReplayBuff,multistepReplayBuff
...
class agent():
    ...
    def __init__(
        ...
        if self.train_mode:
            self.multistep_memory=multistepReplayBuff(replay_capacity,self.env.observation_space.shape[0],self.update_horizon,gamma)
    ...
    def store_transition(self,obs,action,reward,next_obs,done):
        ...
        self.multistep_memory.append(obs,action,reward,next_obs,done)


    def update(self):
        self.optimizer.zero_grad()

        # -------compute multistep loss-------
        multistep_samples=self.multistep_memory.sample(self.batch_size)
        multistep_state=torch.from_numpy(multistep_samples["obs"]).to(self.device)
        multistep_action=torch.from_numpy(multistep_samples["action"].reshape(-1,1)).to(self.device)
        multistep_reward=torch.from_numpy(multistep_samples["reward"].reshape(-1,1)).to(self.device)
        multistep_next_state=torch.from_numpy(multistep_samples["next_obs"]).to(self.device)
        multistep_done=torch.from_numpy(multistep_samples["done"].reshape(-1,1)).to(self.device)
        multistep_indices=multistep_samples["indices"]

        multistep_q_value=self.net(multistep_state).gather(1,multistep_action)
        multistep_next_q_value=self.target_net(
                multistep_next_state
                ).max(dim=1,keepdim=True)[0].detach()
        multistep_mask=1-multistep_done
        # update function,see original paper
        multistep_target=(multistep_reward + self.gamma * multistep_next_q_value * multistep_mask).to(self.device)
        multistep_loss=self.loss_func(multistep_target,multistep_q_value)

        # -------compute one step loss------
        samples=self.memory.sample_from_idxs(multistep_indices)
        ...
        q_value=self.net(state).gather(1,multistep_action)
