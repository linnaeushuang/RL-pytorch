...
class ReplayBuff2(object):
    ...
    def append(self,obs,action,reward,next_obs,terminal,h,c):
        self.observations[self.ptr]=obs
        self.actions[self.ptr]=action
        self.rewards[self.ptr]=reward
        self.next_observations[self.ptr]=next_obs
    ...
    def sample(self,batch_size,horizon):
        ...
        return dict{obs_batch=np.array([self.observations[start:start+horizon] for start in batch_start_idxs]),
                    act_batch=np.array([sef.actions[start:start+horizon] for start in batch_start_idxs]),
                    ...
                    hidden_batch=np.array([self.hidden_state[start:start+horizon] for start in batch_start_idxs]),
                    cell_batch=np.array([self.cell_state[start:start+horizon] for start in batch_start_idxs])}
