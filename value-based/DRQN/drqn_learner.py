...
class agent():
    ...
    def __init__(
        ...
        self.net=Network(self.env.observation_space.shape[0],self.env.action_space.n,self.batch_size).to(self.device)
        if self.train_mode:
            self.memory=ReplayBuff(replay_capacity,self.env.observation_space.shape[0])
            self.target_net=Network(self.env.observation_space.shape[0],self.env.action_space.n,self.batch_size).to(self.device)
    ...
    def select_action(self,state):
        ...
        if self.epsilon < np.random.random():
            with torch.no_grad():
                ...
                action=self.net(state.to(self.device),True).detach().cpu().numpy()
